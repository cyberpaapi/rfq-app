...
function buildComparison(rfq) {
  ...
  const rows = rfq.lines.map((line) => {
    const cells = supIds.map((sid) => {
      ...
      const rate = ql ? Number(ql.rate) : null
      return { sid, rate, total: rate != null ? rate * (Number(line.qty) || 0) : null, note: ql?.remark || '' }
    })
    const valid = cells.filter((c) => c.rate != null)
    const min = valid.length ? Math.min(...valid.map((c) => c.total)) : null
    const winner = valid.find((c) => c.total === min)
    return { line, cells, winnerSid: winner?.sid }
  })
  return { supIds, rows, nameOf }
}
...
export default function Compare() {
  ...
  // Candidates = RFQs with >= 2 supplier responses.
  useEffect(() => {
  ...
  const comparison = useMemo(() => (rfq ? buildComparison(rfq) : null), [rfq])
  ...
  if (candidates.length === 0) {
    return (
      <Card className="p-10 text-center">
        ...
        <p className="text-sm text-ink-400">Comparison needs at least two supplier responses. Submit quotes from the Supplier Portal.</p>
      </Card>
    )
  }

  const { supIds, rows, nameOf } = comparison || { supIds: [], rows: [], nameOf: (x) => x }
  const totalsBySupplier = supIds.map((sid) => ({
    sid,
    total: rows.reduce((a, r) => a + (r.cells.find((x) => x.sid === sid)?.total || 0), 0),
  }))
  const bestTotal = totalsBySupplier.length ? Math.min(...totalsBySupplier.map((t) => t.total)) : 0

  const exportPo = () => window.open(Rfqs.exportPoUrl(rfqId), '_blank')
  const exportCosting = () => window.open(Rfqs.exportCostingUrl(rfqId, oproStock), '_blank')
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button className="btn-outline" onClick={exportPo}><FileSpreadsheet size={16} /> Export to Zoho PO</button>
          <button className="btn-primary" onClick={exportCosting}><Download size={16} /> Costing Sheet</button>
        </div>
      </div>
      ...
      {!rfq ? (
        ...
        <>
              ...
              <table className="w-full min-w-[860px] text-sm">
                <thead>
                  <tr className="border-b border-ink-100 bg-ink-50/60 text-left text-xs font-semibold uppercase tracking-wide text-ink-500">
                    ...
                    {supIds.map((sid) => <th key={sid} className="px-4 py-3 text-right" colSpan={2}>{nameOf(sid)}</th>)}
                  ...
                  {rows.map((row, idx) => (
                    <tr key={row.line.lineId} className="align-top">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-ink-800">{row.line.name}</p>
                        <p className="text-xs text-ink-400">{row.line.spec || row.line.description}</p>
                      </td>
                      <td className="px-4 py-3 text-right text-ink-600">{row.line.qty}</td>
                      {supIds.map((sid) => {
                        const c = row.cells.find((x) => x.sid === sid)
                        const isWinner = row.winnerSid === sid
                        const hasNote = !!c?.note
                        const cellBg = c?.rate == null ? '' : isWinner ? 'bg-emerald-50' : 'bg-amber-50'
                        return (
                          <Fragment key={sid}>
                            <td className={`px-4 py-3 text-right ${cellBg} ${hasNote ? 'border-l-2 border-blue-300' : ''}`}>{c?.rate != null ? fmt(c.rate) : '—'}</td>
                            <td className={`px-4 py-3 text-right font-semibold ${cellBg} ${isWinner ? 'text-emerald-700' : 'text-amber-700'}`}>{c?.total != null ? fmt(c.total) : '—'}</td>
                          </Fragment>
                        )
                      })}
                      <td className="px-4 py-3">
                        {row.winnerSid && <span className="chip bg-emerald-50 text-emerald-700">{nameOf(row.winnerSid).split(' ')[0]}</span>}
                      </td>
                          ...
                          <div className="space-y-1">
                            {row.cells.filter((c) => c.note).map((c) => (
                              <p key={c.sid} className="flex items-start gap-1 rounded-lg bg-blue-50 px-2 py-1 text-xs text-blue-700">
                                ...
                                <span><b>{nameOf(c.sid).split(' ')[0]}:</b> {c.note}</span>
                              </p>
                            ))}
                          </div>
                      ...
                      <td className="px-4 py-3 text-right">
                        <input type="number" min={0} placeholder="0" value={oproStock[row.line.lineId] ?? ''}
                          onChange={(e) => setOproStock((s) => ({ ...s, [row.line.lineId]: e.target.value }))}
                ...
                <tfoot>
                  <tr className="border-t-2 border-ink-100 bg-ink-50/60 font-bold">
                    ...
                    {totalsBySupplier.map((t) => (
                      <td key={t.sid} className={`px-4 py-3 text-right ${t.total === bestTotal ? 'text-emerald-700' : 'text-ink-700'}`} colSpan={2}>{fmt(t.total)}</td>
                    ))}
                    <td className="px-4 py-3 text-emerald-700" colSpan={3}>
                      Best: {nameOf(totalsBySupplier.find((t) => t.total === bestTotal)?.sid)}
                    </td>
                  </tr>
                </tfoot>
              </table>
          ...
          <p className="text-xs text-ink-400">
            Tip: enter required quantities under <b>OPRO Stock</b>, then <b>Costing Sheet</b> exports an Excel with “With OPRO Stock” and “Without OPRO Stock” tabs for Finance approval.
          </p>
        </>
      )}
    </div>
